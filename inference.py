...
def main(args):
    ...
    gt_tensor = imgproc.preprocess_one_image(args.inputs_path, args.upscale_factor, device)

    # Use the model to generate super-resolved images
    with torch.no_grad():
        lr_tensor = sr_model(gt_tensor)

    # Save image
    lr_image = imgproc.tensor_to_image(lr_tensor, False, False)
    lr_image = cv2.cvtColor(lr_image, cv2.COLOR_RGB2BGR)
    cv2.imwrite(args.output_path, lr_image)

    print(f"LR image save to `{args.output_path}`")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inputs_path",
                        type=str,
                        default="./figure/butterfly_gt.png",
                        help="Low-resolution image path.")
    parser.add_argument("--output_path",
                        type=str,
                        default="./figure/butterfly_lr.png",
                        help="Super-resolution image path.")
    parser.add_argument("--model_weights_path",
                        type=str,
                        default="./results/pretrained_models/DSGAN_x4-DF2K_Gaussian.pth.tar",
