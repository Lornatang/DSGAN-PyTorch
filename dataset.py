...
class TrainValidImageDataset(Dataset):
    ...
    Args:
        ...
        gt_image_size (int): Ground-truth image size.
    ...
    def __getitem__(self, batch_index: int) -> [dict[str, Tensor], dict[str, Tensor]]:
        ...
        image = cv2.imread(self.image_file_names[batch_index]).astype(np.float32) / 255.

        # Image processing operations
        if self.mode == "Train":
            image = imgproc.random_crop_np(image, self.gt_image_size)
            image = imgproc.random_rotate(image, [90, 180, 270])
            image = imgproc.random_horizontally_flip(image, 0.5)
            image = imgproc.random_vertically_flip(image, 0.5)
        elif self.mode == "Valid":
            image = imgproc.center_crop(image, self.gt_image_size)
        else:
            raise ValueError("Unsupported data processing model, please use `Train` or `Valid`.")

        lr_image = imgproc.image_resize(image, 1.0 / self.upscale_factor)
        gt_image = imgproc.random_crop_np(image, self.gt_image_size // self.upscale_factor)
        ...
        return {"lr": lr_tensor, "gt": gt_tensor}
...
class TestImageDataset(Dataset):
    ...
    def __init__(self, test_lr_images_dir: str, upscale_factor: int, test_gt_images_dir: str = None) -> None:
        super(TestImageDataset, self).__init__()
        self.upscale_factor = upscale_factor
        self.test_gt_images_dir = test_gt_images_dir
        # Get all image file names in folder
        self.lr_image_file_names = [os.path.join(test_lr_images_dir, x) for x in os.listdir(test_lr_images_dir)]
        if test_gt_images_dir is not None:
    ...
    def __getitem__(self, batch_index: int) -> [Tensor, Tensor]:
        ...
        crop_image_size = crop_size - (crop_size % self.upscale_factor)

        # Create all images
        lr_image = imgproc.center_crop(lr_image, crop_image_size)
        lr_image = imgproc.image_resize(lr_image, 1.0 / self.upscale_factor)

        if self.test_gt_images_dir is not None:
            # Read a batch of gt image data
            gt_image = cv2.imread(self.gt_image_file_names[batch_index]).astype(np.float32) / 255.
            gt_image = imgproc.center_crop(gt_image, crop_image_size // self.upscale_factor)
        else:
            gt_image = lr_image
